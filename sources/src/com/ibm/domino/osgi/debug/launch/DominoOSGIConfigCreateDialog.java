 ...
 * @author dtaieb
 * @author doconnor
 * The dialog that will be popped up when creating an OSGi framework configuration
 *
 */
public class DominoOSGIConfigCreateDialog extends TitleAreaDialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        String prefBinValue = Activator.getDefault().getPreferenceStore().getString(PREF_DOMINO_BIN_DIR);
        String prefDataValue = Activator.getDefault().getPreferenceStore().getString(PREF_DOMINO_DATA_DIR);
    ...
    protected void updateControls() {
        try {
            ...
            if (config.getDominoBinPath() != null && config.getDominoBinPath().length() > 0) {
                Activator.getDefault().getPreferenceStore().setValue(PREF_DOMINO_BIN_DIR, config.getDominoBinPath());
            }
            if (config.getDominoDataPath() != null && config.getDominoDataPath().length() > 0) {
                Activator.getDefault().getPreferenceStore().setValue(PREF_DOMINO_DATA_DIR, config.getDominoDataPath());
            }
            config.isValid();
